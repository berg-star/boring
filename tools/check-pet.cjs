...
(async()=>{
 ...
 await p.evaluate(k=>{const s=JSON.parse(localStorage.getItem(k));s.awayUntil=Date.now()+1000;localStorage.setItem(k,JSON.stringify(s));},key);await p.reload();assert.equal(await p.locator('#away-note').isVisible(),true);await p.waitForTimeout(1100);await p.evaluate(()=>document.dispatchEvent(new Event('visibilitychange')));assert.equal(await p.locator('#pet').isVisible(),true);
