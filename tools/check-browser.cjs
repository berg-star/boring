...
async function main() {
  ...
  try {
    ...
    const pages = ['','reaction.html','wheel.html','card.html','question.html','fun.html','truth.html','pet.html','planet.html'];
    ...
    await page.goto(base);assert.equal(await page.locator('.game-card').count(),8);
    ...
    await page.getByRole('button',{name:/随便给我来一个/}).click();await page.waitForURL(/\/(reaction|wheel|card|question|fun|truth|pet|planet)\.html$/);
    ...
    console.log('PASS: APIs, random variety, missing routes, resources, all eight games, keyboard, wheel pointer, retry, 320/390/768px layouts, invalid data.');
