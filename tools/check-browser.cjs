...
async function main() {
  ...
  try {
    ...
    await page.goto(base);assert.equal(await page.locator('.game-card').count(),9);
    ...
    await page.getByRole('button',{name:/随便给我来一个/}).click();await page.waitForURL(/\/(reaction|wheel|card|question|fun|truth|pet|planet|book)\.html$/);
    ...
    console.log('PASS: APIs, random variety, missing routes, resources, all nine games, keyboard, wheel pointer, retry, 320/390/768px layouts, invalid data.');
