...
`smash.html` / `smash.js` / `smash.css` 提供玻璃、积木、气泡三个场景，无分数和时间限制。玻璃三次有效敲击后碎裂；积木使用轻量二维重力、轴对齐矩形碰撞和拖拽释放速度（不模拟旋转刚体）；气泡按滑动轨迹采样，快速划过也能连续戳破。每个场景均支持重置和键盘/按钮操作。

Web Audio 实时合成音效，限制音量、发声频率和并发数量；关闭音效立即停止现有声音。震动默认关闭，仅支持的浏览器可开启，实际反馈取决于设备。偏好存于 `boring-lab-smash-settings-v1`，不保存破坏进度。页面隐藏时暂停动画并停止声振；减少动态效果设置下跳过玻璃飞散和气泡扩散动画，积木保留核心物理交互。首页现有 11 个玩法。

验收：`node tools/check-smash.cjs`，可用 `BASE_URL` 检查公网版本。浏览器测试验证音频节点生成和开关，不等于对扬声器音质或真机马达强度的实测。
